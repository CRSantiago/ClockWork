      ...
      User.find({ username: uname }, function (err, data) {
        ...
        bcrypt.compare(pass, data[0].password, (err, match) => {
          ...
          if (!data[0].isVerified) {
            // NOT VERIFIED
            error = 'User is not verified. Please check your email to verify.'
            console.log(error)
          ...
          if (match) {
            ...
            username_t = uname
            _id_t = data[0]._id
            token_t = jwt.sign({ username_t }, process.env.JWT_SECRET_KEY, {
              expiresIn: 86400,
            })
            resolve({ _id_t, username_t, password_t, token_t, error })
          } else {
            error =
              'Incorrect password. Please check your credentials or reset your password below.'
            console.log(error)
            resolve({ _id_t, username_t, password_t, token_t, error })
          }
        })
  ...
  static async usersRegister(uname, pass, uemail, err) {
    ...
    const jsonval = new Promise((resolve, reject) => {
      ...
      User.find({ username: uname }, function (err, data) {
        ...
        if (data.length == 0) {
          User.find({ email: uemail }, function (err, data) {
            // check email
            if (err) {
              console.log(err)
            }
            if (data.length == 0) {
                ...
                bcrypt.hash(pass, salt, (err, hash) => {
                  ...
                  newUser.save((err, user) => {
                    if (err) {
                      ...
                      reject(
                        new Error(
                          'Error when saving user to database in usersRegister api'
                        )
                      )
                    } else {
                      console.log(user)
              ...
              error =
                'Email is already in use. Please try using a different email.'
              console.log(error)
              resolve({ username_t, password_t, email_t, error })
            }
          })
